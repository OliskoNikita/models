...
def calculate_iou(pred, target):
    ...
    num_classes=2
    iou_per_class = []
    pred = pred.argmax(dim=1).view(-1)  # Преобразуем в вектор класса
    target = target.view(-1)            # Преобразуем в вектор класса
...
def calculate_pixel_accuracy(pred, target):
    ...
    """
    pred = pred.argmax(dim=1)  # Преобразуем в прогнозируемый класс
...
def test():
    ...
    checkpoint = torch.load(f"C:/models1/PANmodel/PAN_mobilenet_v2_{Epoch}.pth")
